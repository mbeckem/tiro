...
class FunctionCodegen {
public:
    explicit FunctionCodegen(ast::FuncDecl& func, ModuleCodegen& module, StringTable& strings_,
                             Diagnostics& diag_);

    void compile();

    std::unique_ptr<CompiledFunction> take_result() { return std::move(result_); }
...
private:
    ...
    void compile_member_assign(ast::DotExpr* lhs, ast::Expr* rhs, bool push_value);
    void compile_index_assign(ast::IndexExpr* lhs, ast::Expr* rhs, bool push_value);
...
private:
    ...
    struct ConstantCompare {
        bool operator()(const CompiledOutput* o1, const CompiledOutput* o2) const noexcept {
...
private:
    ...
    std::unordered_map<const CompiledOutput*, u32, ConstantHasher, ConstantCompare>
        constant_to_index_;
