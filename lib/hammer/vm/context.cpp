...
static i64 to_integer(Handle<Value> v) {
    ...
    default:
        // TODO exception
        HAMMER_ERROR("Cannot convert value of type {} to integer.", to_string(v->type()));
    }
}
...
static double to_float(Handle<Value> v) {
    ...
    default:
        // TODO exception
        HAMMER_ERROR("Cannot convert value of type {} to float.", to_string(v->type()));
    }
}
...
static Value binary_op(Context& ctx, Handle<Value> left, Handle<Value> right, Operation&& op) {
    if (left->is<Float>() || right->is<Float>()) {
        double a = left->is<Float>() ? left->as<Float>().value() : to_float(left);
        double b = right->is<Float>() ? right->as<Float>().value() : to_float(right);
        return Float::make(ctx, op(a, b));
    } else {
        i64 a = left->is<Integer>() ? left->as<Integer>().value() : to_integer(left);
        i64 b = right->is<Integer>() ? right->as<Integer>().value() : to_integer(right);
...
static Value bitwise_not(Context& ctx, Handle<Value> v) {
    if (HAMMER_UNLIKELY(!v->is<Integer>()))
        HAMMER_ERROR("Invalid operand type for bitwise not: {}.", to_string(v->type()));
...
static void unary_plus(Handle<Value> v) {
    ...
    case ValueType::Float:
        return;
    default:
        HAMMER_ERROR("Invalid operand type for unary plus: {}.", to_string(v->type()));
    }
}
...
static Value unary_minus(Context& ctx, Handle<Value> v) {
    ...
    default:
        HAMMER_ERROR("Invalid operand type for unary minus: {}.", to_string(v->type()));
    }
}
...
static int compare(Handle<Value> a, Handle<Value> b) {
    ...
    HAMMER_ERROR("Comparisons are not defined for types {} and {}.", to_string(a->type()),
                 to_string(b->type()));
}
...
Module Context::load(const CompiledModule& compiled_module, const StringTable& strings) {
    Root module_name(*this, String::make(*this, strings.value(compiled_module.name)));
    Root module_members(*this, Array::make(*this, compiled_module.members.size()));
    Root module(*this, Module::make(*this, module_name, module_members));

    size_t index = 0;
    for (const auto& member_ptr : compiled_module.members) {
        Overloaded visitor{
            [&](const CompiledImport& i) {
                unused(i);
                // FIXME imports
                HAMMER_ERROR("Imports not implemented yet.");
            },
            [&](const CompiledFunction& f) {
                Root function_name(*this, String::make(*this, strings.value(f.name)));
                Root literals(*this, Array::make(*this, f.literals.size()));

                Root tmpl(*this, FunctionTemplate::make(*this, function_name, module, literals,
                                                        f.params, f.locals, f.code));
...
Value Context::run(Handle<Function> fn) {
    ...
    HAMMER_CHECK(fn->tmpl().params() == 0, "Can only invoke nullary functions right now.");
...
Value Context::run_until_complete(Handle<Coroutine> coro) {
    ...
    HAMMER_ASSERT(coro->state() == CoroutineState::Ready, "Cannot run coroutines with this state.");

    while (coro->stack().top_frame() != nullptr) {
        run_frame(coro);
    }

    HAMMER_ASSERT(coro->stack().top_value_count() == 1, "Must have left one value on the stack.");
    HAMMER_WRITE_MEMBER(*this, *coro, result, Handle<Value>::from_slot(coro->stack().top_value()));
...
void Context::run_frame(Handle<Coroutine> coro) {
    ...
    auto grow_stack = [&]() {
        u32 next_size;
        if (HAMMER_UNLIKELY(!checked_mul<u32>(stack.stack_size(), 2, next_size)))
        ...
        Root new_stack(*this, CoroutineStack::grow(*this, old_stack, next_size));
    ...
    auto read_op = [&] {
        ...
        HAMMER_ASSERT(*frame->pc != 0 && *frame->pc <= static_cast<u8>(Opcode::last_opcode),
                      "Invalid opcode.");
    ...
    while (1) {
        ...
        if (HAMMER_UNLIKELY(frame->pc == code.end()))
            HAMMER_ERROR(
                "Invalid program counter: end of code reached without return from "
                "function.");
        ...
        case Opcode::load_const: {
            ...
            // TODO static verify
            HAMMER_ASSERT(literals && index < literals.size(), "Invalid constant index.");
            push_value(literals.get(index));
            break;
        }
        case Opcode::load_param: {
            u32 index = read_u32();
            HAMMER_ASSERT(index < frame->args, "Parameter index out of bounds.");
        ...
        case Opcode::load_index: {
            ...
            HAMMER_CHECK(array_value.is<Array>(), "The value is not an array."); // TODO nicer

            Value index_value = *stack.top_value(0);
            HAMMER_CHECK(index_value.is<Integer>(), "The value is not an integer."); // TODO nicer
        ...
        case Opcode::load_module: {
            ...
            // TODO static verify
            HAMMER_ASSERT(members && index < members.size(), "Module member index out of bounds.");
