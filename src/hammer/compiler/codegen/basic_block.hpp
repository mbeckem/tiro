...
public:
    ...
    const None& none() const {
        HAMMER_ASSERT(which_ == Which::None, "Invalid access.");
        return none_;
    }

    const Jump& jump() const {
        HAMMER_ASSERT(which_ == Which::Jump, "Invalid access.");
        return jump_;
    }

    const CondJump& cond_jump() const {
        HAMMER_ASSERT(which_ == Which::CondJump, "Invalid access.");
        return cond_jump_;
    }

    const Ret& ret() const {
        HAMMER_ASSERT(which_ == Which::Ret, "Invalid access.");
        return ret_;
    }
