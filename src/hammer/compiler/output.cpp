...
ModuleItem& ModuleItem::operator=(ModuleItem&& other) noexcept {
    HAMMER_CHECK(this != &other, "Self move assignment.");
...
bool ModuleItem::operator==(const ModuleItem& other) const noexcept {
    ...
    switch (which_) {
#define HAMMER_COMPARE(Type, var, _a) \
    case Which::Type:                 \
        return var == other.var;

        HAMMER_MODULE_ITEMS(HAMMER_COMPARE)

#undef HAMMER_COMPARE
    }
...
std::string_view to_string(ModuleItem::Which which) {
    switch (which) {
#define HAMMER_TO_STR(Type, _v, _a) \
    case ModuleItem::Which::Type:   \
        return #Type;

        HAMMER_MODULE_ITEMS(HAMMER_TO_STR)
...
std::string_view to_string(FunctionDescriptor::Type type) {
    ...
    HAMMER_UNREACHABLE("Invalid function type.");
}
