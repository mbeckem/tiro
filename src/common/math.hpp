...
constexpr T ceil(T a, T b) noexcept {
    TIRO_DEBUG_ASSERT(b != 0, "b must not be 0.");
    return ((a + b - 1) / b) * b;
}
...
constexpr T mod_pow2(T a, T b) noexcept {
    TIRO_DEBUG_ASSERT(is_pow2(b), "b must be a power of two");
    return a & (b - 1);
}
...
constexpr T ceil_div(T a, T b) {
    TIRO_DEBUG_ASSERT(a >= 0, "Dividend must be greater than or equal to zero.");
    TIRO_DEBUG_ASSERT(b > 0, "Divisor must be greater than zero.");
    return (a + b - 1) / b;
}
