#include "tiro/mir_gen/gen_rvalue.hpp"
...
static RValue::BinaryOp
commutative_order(const RValue::BinaryOp& binop) {
...
RValueMIRGen::RValueMIRGen(FunctionMIRGen& ctx, BlockID blockID)
    : ctx_(ctx)
    , blockID_(blockID) {}

RValueMIRGen::~RValueMIRGen() {}

LocalID RValueMIRGen::compile(const RValue& value) {
    return value.visit(*this);
}

LocalID RValueMIRGen::visit_use_lvalue(const RValue::UseLValue& use) {
...
LocalID RValueMIRGen::visit_use_local(const RValue::UseLocal& use) {
...
LocalID RValueMIRGen::visit_phi(const RValue::Phi& phi) {
...
LocalID RValueMIRGen::visit_phi0(const RValue::Phi0& phi) {
    // See visit_phi().
    return define_new(phi);
}

LocalID RValueMIRGen::visit_constant(const Constant& constant) {
    const auto key = ComputedValue::make_constant(constant);
    return memoize_value(key, [&]() { return define_new(constant); });
}

LocalID RValueMIRGen::visit_outer_environment(
    [[maybe_unused]] const RValue::OuterEnvironment& env) {
    return compile_env(ctx().outer_env());
}

LocalID
RValueMIRGen::visit_binary_op(const RValue::BinaryOp& original_binop) {
...
LocalID RValueMIRGen::visit_unary_op(const RValue::UnaryOp& unop) {
...
LocalID RValueMIRGen::visit_call(const RValue::Call& call) {
    return define_new(call);
}

LocalID
RValueMIRGen::visit_method_handle(const RValue::MethodHandle& method) {
...
LocalID
RValueMIRGen::visit_method_call(const RValue::MethodCall& call) {
    TIRO_ASSERT(value_of(call.method).type() == RValueType::MethodHandle,
        "method must be a MethodHandle.");
    return define_new(call);
}

LocalID RValueMIRGen::visit_make_environment(
    const RValue::MakeEnvironment& make_env) {
    return define_new(make_env);
}

LocalID
RValueMIRGen::visit_make_closure(const RValue::MakeClosure& make_closure) {
    return define_new(make_closure);
}

LocalID RValueMIRGen::visit_container(const RValue::Container& cont) {
    return define_new(cont);
}

LocalID RValueMIRGen::visit_format(const RValue::Format& format) {
...
std::optional<Constant> RValueMIRGen::try_eval_binary(
    BinaryOpType op, LocalID lhs, LocalID rhs) {
...
RValueMIRGen::try_eval_unary(UnaryOpType op, LocalID local) {
...
void RValueMIRGen::report(std::string_view which, const EvalResult& result) {
    switch (result.type()) {
...
LocalID RValueMIRGen::compile_env(ClosureEnvID env) {
    return ctx().compile_env(env, blockID_);
}

LocalID RValueMIRGen::define_new(const RValue& value) {
    return ctx().define_new(value, blockID_);
}

LocalID RValueMIRGen::memoize_value(
    const ComputedValue& key, FunctionRef<LocalID()> compute) {
    return ctx().memoize_value(key, compute, blockID_);
}

RValue RValueMIRGen::value_of(LocalID local) const {
