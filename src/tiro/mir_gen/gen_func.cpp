...
FunctionMIRGen::FunctionMIRGen(ModuleMIRGen& module,
...
const LoopContext* FunctionMIRGen::current_loop() const {
    return active_loops_.empty() ? nullptr : &active_loops_.back();
}

ClosureEnvID FunctionMIRGen::current_env() const {
...
ExprResult FunctionMIRGen::compile_expr(
    NotNull<Expr*> expr, CurrentBlock& bb, ExprOptions options) {
    ExprMIRGen transformer(*this, bb);
...
StmtResult
FunctionMIRGen::compile_stmt(NotNull<ASTStmt*> stmt, CurrentBlock& bb) {
    StmtMIRGen transformer(*this, bb);
...
void FunctionMIRGen::compile_assign(
...
BlockID FunctionMIRGen::make_block(InternedString label) {
...
void FunctionMIRGen::seal(BlockID blockID) {
...
void FunctionMIRGen::emit(const Stmt& stmt, BlockID blockID) {
    auto block = result_[blockID];
...
void FunctionMIRGen::write_variable(
...
void FunctionMIRGen::add_phi_operands(
    NotNull<Symbol*> var, LocalID value, BlockID blockID) {
    auto block = result_[blockID];
...
std::optional<LocalID> FunctionMIRGen::find_env(ClosureEnvID env) {
...
LocalID FunctionMIRGen::get_env(ClosureEnvID env) {
