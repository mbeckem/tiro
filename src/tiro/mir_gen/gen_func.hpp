#ifndef TIRO_MIR_GEN_GEN_FUNC_HPP
#define TIRO_MIR_GEN_GEN_FUNC_HPP
...
#include "tiro/mir_gen/fwd.hpp"
#include "tiro/mir_gen/support.hpp"
...
public:
    CurrentBlock(FunctionMIRGen& ctx, BlockID id)
        : ctx_(ctx)
        , id_(id) {
...
class FunctionMIRGen final {
public:
    explicit FunctionMIRGen(ModuleMIRGen& module,
        NotNull<ClosureEnvCollection*> envs, ClosureEnvID closure_env,
        Function& result, Diagnostics& diag, StringTable& strings);

    FunctionMIRGen(const FunctionMIRGen&) = delete;
    FunctionMIRGen& operator=(const FunctionMIRGen&) = delete;

    ModuleMIRGen& module() const { return module_; }
...
class Transformer {
public:
    Transformer(FunctionMIRGen& ctx, CurrentBlock& bb)
        : ctx_(ctx)
        , bb_(bb) {}
...
#endif // TIRO_MIR_GEN_GEN_FUNC_HPP
