...
ModuleMIRGen::ModuleMIRGen(NotNull<Root*> module, Module& result,
...
void ModuleMIRGen::compile_module() {
    while (!jobs_.empty()) {
...
ModuleMemberID ModuleMIRGen::find_symbol(NotNull<Symbol*> symbol) const {
    if (auto pos = members_.find(symbol); pos != members_.end())
        return pos->second;
    return {};
}

ModuleMemberID ModuleMIRGen::add_function(NotNull<FuncDecl*> decl,
