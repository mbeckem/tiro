#ifndef TIRO_COMPILER_SYNTAX_AST_HPP
#define TIRO_COMPILER_SYNTAX_AST_HPP
...
/// Use TIRO_VISITOR_OVERRIDE when you inherit from DefaultNodeVisitor
...
#ifdef TIRO_DEBUG
#    define TIRO_VISITOR_DECL virtual
#    define TIRO_VISITOR_OVERRIDE override
#else
#    define TIRO_VISITOR_DECL
#    define TIRO_VISITOR_OVERRIDE
#endif
...
TIRO_FORCE_INLINE bool isa(From* from) {
...
template<typename To, typename From>
TIRO_FORCE_INLINE bool isa(const Ref<From>& from) {
...
template<typename To, typename From>
TIRO_FORCE_INLINE To* try_cast(From* from) {
    return isa<To>(from) ? static_cast<To*>(from) : nullptr;
}

template<typename To, typename From>
TIRO_FORCE_INLINE Ref<To> try_cast(const Ref<From>& from) {
...
template<typename To, typename From>
TIRO_FORCE_INLINE To* must_cast_nullable(From* from) {
    return from ? must_cast<To>(from) : nullptr;
}

template<typename To, typename From>
TIRO_FORCE_INLINE Ref<To> must_cast_nullable(const Ref<From>& from) {
...
#include "tiro/compiler/syntax/ast.generated.hpp"
...
#include "tiro/compiler/syntax/ast.generated.ipp"

#endif // TIRO_COMPILER_SYNTAX_AST_HPP
