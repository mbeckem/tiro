...
static bool
has_side_effects(const RValue& value, const Function& func) {
    struct Visitor {
        const Function& func;
        ...
        bool visit_make_closure(const RValue::MakeClosure&) {
            return false;
        }
...
LocalVisitor::LocalVisitor(
    const Function& func, FunctionRef<void(LocalID)> cb)
    : func_(func)
    , cb_(cb) {
