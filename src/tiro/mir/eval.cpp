...
EvalResult::EvalResult(const mir::Constant& value)
...
static bool is_integer(const mir::Constant& c) {
    return c.type() == mir::ConstantType::Integer;
}

static bool is_float(const mir::Constant& c) {
    return c.type() == mir::ConstantType::Float;
}

static bool is_numeric(const mir::Constant& c) {
    return is_integer(c) || is_float(c);
}

static i64 get_int(const mir::Constant& c) {
    return c.as_integer().value;
}

static f64 get_float(const mir::Constant& c) {
    return c.as_float().value;
}

static f64 convert_float(const mir::Constant& c) {
...
static bool is_equal(const mir::Constant& lhs, const mir::Constant& rhs) {
...
static mir::Constant make_int(i64 value) {
    return mir::Constant::make_integer(value);
}

static mir::Constant make_float(f64 value) {
    return mir::Constant::make_float(value);
}

static mir::Constant make_bool(bool value) {
    return value ? mir::Constant::make_true() : mir::Constant::make_false();
}

template<typename IntOp, typename FloatOp>
static EvalResult numeric_op(const mir::Constant& lhs, const mir::Constant& rhs,
...
static EvalResult
integer_op(const mir::Constant& lhs, const mir::Constant& rhs, IntOp&& op) {
    if (!is_integer(lhs) || !is_integer(rhs))
        return EvalResult::make_type_error();
...
static EvalResult
eval_plus(const mir::Constant& lhs, const mir::Constant& rhs) {
...
static EvalResult
eval_minus(const mir::Constant& lhs, const mir::Constant& rhs) {
    constexpr auto intop = [](i64 a, i64 b) -> EvalResult {
        SafeInt result = a;
        if (!result.try_sub(b))
            return EvalResult::make_integer_overflow();
...
static EvalResult
eval_multiply(const mir::Constant& lhs, const mir::Constant& rhs) {
    constexpr auto intop = [](i64 a, i64 b) -> EvalResult {
        SafeInt result = a;
        if (!result.try_mul(b))
            return EvalResult::make_integer_overflow();
...
static EvalResult
eval_divide(const mir::Constant& lhs, const mir::Constant& rhs) {
...
static EvalResult
eval_remainder(const mir::Constant& lhs, const mir::Constant& rhs) {
    static constexpr auto intop = [](i64 a, i64 b) -> EvalResult {
        if (b == 0)
            return EvalResult::make_divide_by_zero();
...
static EvalResult
eval_power(const mir::Constant& lhs, const mir::Constant& rhs) {
    constexpr auto intop = [](i64 a, i64 b) -> EvalResult {
        if (a == 0 && b < 0)
            return EvalResult::make_divide_by_zero();
        if (b == 0)
...
static EvalResult
eval_left_shift(const mir::Constant& lhs, const mir::Constant& rhs) {
...
static EvalResult
eval_right_shift(const mir::Constant& lhs, const mir::Constant& rhs) {
    return integer_op(lhs, rhs, [&](i64 a, i64 b) -> EvalResult {
        if (b < 0)
            return EvalResult::make_negative_shift();
        if (b > max_shift)
...
static EvalResult
eval_bitwise_and(const mir::Constant& lhs, const mir::Constant& rhs) {
    return integer_op(lhs, rhs, [&](i64 a, i64 b) { return make_int(a & b); });
}

static EvalResult
eval_bitwise_or(const mir::Constant& lhs, const mir::Constant& rhs) {
    return integer_op(lhs, rhs, [&](i64 a, i64 b) { return make_int(a | b); });
}

static EvalResult
eval_bitwise_xor(const mir::Constant& lhs, const mir::Constant& rhs) {
    return integer_op(lhs, rhs, [&](i64 a, i64 b) { return make_int(a ^ b); });
}

static EvalResult
eval_equals(const mir::Constant& lhs, const mir::Constant& rhs) {
    return make_bool(is_equal(lhs, rhs));
}

static EvalResult
eval_not_equals(const mir::Constant& lhs, const mir::Constant& rhs) {
    return make_bool(!is_equal(lhs, rhs));
}

template<typename Test>
static EvalResult numeric_compare(
    const mir::Constant& lhs, const mir::Constant& rhs, Test&& test) {
...
static EvalResult
eval_less(const mir::Constant& lhs, const mir::Constant& rhs) {
    return numeric_compare(lhs, rhs, [](int cmp) { return cmp < 0; });
}

static EvalResult
eval_less_equals(const mir::Constant& lhs, const mir::Constant& rhs) {
    return numeric_compare(lhs, rhs, [](int cmp) { return cmp <= 0; });
}

static EvalResult
eval_greater(const mir::Constant& lhs, const mir::Constant& rhs) {
    return numeric_compare(lhs, rhs, [](int cmp) { return cmp > 0; });
}

static EvalResult
eval_greater_equals(const mir::Constant& lhs, const mir::Constant& rhs) {
    return numeric_compare(lhs, rhs, [](int cmp) { return cmp >= 0; });
}

static EvalResult eval_plus(const mir::Constant& value) {
    if (!is_numeric(value))
        return EvalResult::make_type_error();
...
static EvalResult eval_minus(const mir::Constant& value) {
    if (!is_numeric(value))
        return EvalResult::make_type_error();
...
static EvalResult eval_bitwise_not(const mir::Constant& value) {
    if (!is_integer(value))
        return EvalResult::make_type_error();
...
static EvalResult eval_logical_not(const mir::Constant& value) {
    return make_bool(value.type() == mir::ConstantType::Null
                     || value.type() == mir::ConstantType::False);
}
...
eval_format(Span<const mir::Constant> operands, StringTable& strings) {
    struct Visitor {
        ...
        void visit_integer(const mir::Constant::Integer& i) {
            stream.format("{}", i.value);
        }

        void visit_float(const mir::Constant::Float& f) {
            // TODO: need precise format specifier (same format used at runtime)
            stream.format("{}", f.value);
        }

        void visit_string(const mir::Constant::String& s) {
            stream.format("{}", strings.value(s.value));
        }

        void visit_symbol(const mir::Constant::Symbol& s) {
            stream.format("#{}", strings.value(s.value));
        }

        void visit_null(const mir::Constant::Null&) { stream.format("null"); }

        void visit_true(const mir::Constant::True&) { stream.format("true"); }

        void visit_false(const mir::Constant::False&) {
            stream.format("false");
        }
    };
    ...
    return mir::Constant::make_string(strings.insert(result));
}
