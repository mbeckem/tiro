...
void BytecodeBuilder::emit(const BytecodeInstr& ins) {
    struct Visitor {
        ...
        void visit_load_null(const BytecodeInstr::LoadNull& n) {
            self.write(op, n.target);
        }

        void visit_load_false(const BytecodeInstr::LoadFalse& f) {
            self.write(op, f.target);
        }

        void visit_load_true(const BytecodeInstr::LoadTrue& t) {
        ...
        void
        visit_store_tuple_member(const BytecodeInstr::StoreTupleMember& t) {
        ...
        void visit_load_closure(const BytecodeInstr::LoadClosure& c) {
        ...
        void visit_add(const BytecodeInstr::Add& a) {
            self.write(op, a.lhs, a.rhs, a.target);
        }

        void visit_sub(const BytecodeInstr::Sub& s) {
            self.write(op, s.lhs, s.rhs, s.target);
        }

        void visit_mul(const BytecodeInstr::Mul& m) {
            self.write(op, m.lhs, m.rhs, m.target);
        }

        void visit_div(const BytecodeInstr::Div& d) {
            self.write(op, d.lhs, d.rhs, d.target);
        }

        void visit_mod(const BytecodeInstr::Mod& m) {
            self.write(op, m.lhs, m.rhs, m.target);
        }

        void visit_pow(const BytecodeInstr::Pow& p) {
            self.write(op, p.lhs, p.rhs, p.target);
        }

        void visit_uadd(const BytecodeInstr::UAdd& a) {
            self.write(op, a.value, a.target);
        }

        void visit_uneg(const BytecodeInstr::UNeg& n) {
            self.write(op, n.value, n.target);
        }

        void visit_lsh(const BytecodeInstr::LSh& s) {
            self.write(op, s.lhs, s.rhs, s.target);
        }

        void visit_rsh(const BytecodeInstr::RSh& s) {
            self.write(op, s.lhs, s.rhs, s.target);
        }

        void visit_band(const BytecodeInstr::BAnd& a) {
            self.write(op, a.lhs, a.rhs, a.target);
        }

        void visit_bor(const BytecodeInstr::BOr& o) {
            self.write(op, o.lhs, o.rhs, o.target);
        }

        void visit_bxor(const BytecodeInstr::BXor& x) {
            self.write(op, x.lhs, x.rhs, x.target);
        }

        void visit_bnot(const BytecodeInstr::BNot& n) {
            self.write(op, n.value, n.target);
        }

        void visit_gt(const BytecodeInstr::Gt& g) {
            self.write(op, g.lhs, g.rhs, g.target);
        }

        void visit_gte(const BytecodeInstr::Gte& g) {
            self.write(op, g.lhs, g.rhs, g.target);
        }

        void visit_lt(const BytecodeInstr::Lt& l) {
            self.write(op, l.lhs, l.rhs, l.target);
        }

        void visit_lte(const BytecodeInstr::Lte& l) {
            self.write(op, l.lhs, l.rhs, l.target);
        }

        void visit_eq(const BytecodeInstr::Eq& e) {
            self.write(op, e.lhs, e.rhs, e.target);
        }

        void visit_neq(const BytecodeInstr::NEq& n) {
            self.write(op, n.lhs, n.rhs, n.target);
        }

        void visit_lnot(const BytecodeInstr::LNot& n) {
            self.write(op, n.value, n.target);
        }

        void visit_array(const BytecodeInstr::Array& a) {
            self.write(op, a.count, a.target);
        }

        void visit_tuple(const BytecodeInstr::Tuple& t) {
            self.write(op, t.count, t.target);
        }

        void visit_set(const BytecodeInstr::Set& s) {
            self.write(op, s.count, s.target);
        }

        void visit_map(const BytecodeInstr::Map& m) {
            self.write(op, m.count, m.target);
        }

        void visit_env(const BytecodeInstr::Env& e) {
        ...
        void visit_formatter(const BytecodeInstr::Formatter& f) {
            self.write(op, f.target);
        }
        ...
        void visit_copy(const BytecodeInstr::Copy& c) {
            self.write(op, c.source, c.target);
        }

        void visit_swap(const BytecodeInstr::Swap& s) {
            self.write(op, s.a, s.b);
        }

        void visit_push(const BytecodeInstr::Push& p) {
            self.write(op, p.value);
        }

        void visit_pop(const BytecodeInstr::Pop&) { self.write(op); }

        void visit_pop_to(const BytecodeInstr::PopTo& p) {
        ...
        void visit_return(const BytecodeInstr::Return& r) {
