...
public:
    ...
    struct Message {
        ...
        Message(Level level_, const AbsoluteSourceRange& range_, std::string text_)
            : level(level_)
            , range(range_)
            , text(std::move(text_)) {}
    };
...
public:
    ...
    void report(Level level, const AbsoluteSourceRange& range, std::string text);

    void vreport(Level level, const AbsoluteSourceRange& range, std::string_view format_string,
        fmt::format_args format_args);

    /// Report a message at the given source text location, with fmt::format syntax.
    template<typename... Args>
    void reportf(Level level, const AbsoluteSourceRange& range, std::string_view format_string,
