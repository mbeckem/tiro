...
/// TODO Tests
inline void* aligned_container_from_member(void* member, size_t container_alignment) {
    // TODO: This is technically undefined behaviour, even though must platforms allow it.
    // Memory is not necessarly linear when reinterpreted as uintptr_t.
    uintptr_t raw_member = reinterpret_cast<uintptr_t>(member);
    return reinterpret_cast<void*>(raw_member & aligned_container_mask(container_alignment));
}
