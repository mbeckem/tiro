...
auto make_dynamic(size_t capacity, Args&&... args) {
    ...
    size_t alloc_size = Traits::dynamic_alloc_size(capacity);
...
TEST_CASE("Fixed slots layout should support tracing", "[layout]") {
    ...
    REQUIRE(Traits::dynamic_size(object.get()) == Traits::dynamic_alloc_size(dynamic_elements));
...
TEST_CASE("Dynamic slots layout should support tracing", "[layout]") {
    ...
    REQUIRE(Traits::dynamic_size(object.get()) == Traits::dynamic_alloc_size(dynamic_capacity));
...
TEST_CASE("Buffer layout should construct a valid buffer", "[layout]") {
    ...
    REQUIRE(Traits::dynamic_size(object.get()) == Traits::dynamic_alloc_size(buffer_capacity));
