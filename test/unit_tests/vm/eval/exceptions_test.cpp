...
TEST_CASE("Panics should be registered as secondary exceptions if another exception is in flight",
    ...
    require_message(root_secondaries->checked_get(0), "test-secondary-2");
    require_message(root_secondaries->checked_get(1), "nested-panic");
    require_message(root_secondaries->checked_get(2), "test-secondary-1");

    Local nested_exception = sc.local(root_secondaries->checked_get(1).must_cast<Exception>());
    ...
    require_message(nested_secondaries->checked_get(0), "nested-secondary-1");
}
